...
function BookingScreen() {
    ...
    async function bookRoom(transaction) {
        ...
        await axios.post("http://localhost:4000/api/bookings/bookRoom", bookingDetails).then(
            (response) => {
                Swal.fire({
    ...
    //     }
    //     await axios.post("http://localhost:4000/api/bookings/payment", stripeToken).then(
    //         (response) => {
    ...
    // remove StripeCheckout
// add a simple button that calls bookRoom()
...
async function bookRoom() {
    const bookingDetails = {
        ...
        totalAmount: totalDays * room.pricePerDay,
        totalDays,
        transactionId: "TEMP-NO-PAYMENT"  // dummy transaction id
    };
    ...
    await axios.post("http://localhost:4000/api/bookings/bookRoom", bookingDetails).then(
        (response) => {
            Swal.fire({
                title: "Your booking is successful",
                text: "Please check your profile for details",
                icon: "success",
            }).then(() => {
                window.location.href = "/profile";
            });
                    ...
                    <div className="row">
                        ...
                        <div className="col-md-6">
                            ...
                            {/* <StripeCheckout
                                ...
                                currency="LKR"
                                token={onToken}
                                stripeKey="pk_test_51RnbkmPSxFdLoiS79ap90TKCUwQVGhnnQnT9AmoS1JAQpoA4LQr8wfCBiCOkrU3VjzFn7GWB5Mu0khX7JG0LuYhD00JfdsmCtP"
                            >
                            <button
